...
class EmixilDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some emixild methods
    ...
    @classmethod
    def from_emixil_conf(self, emixil_dot_conf):
        from emixil_config import EmixilConfig
        config_text = EmixilConfig.slurp_config_file(emixil_dot_conf)
        creds = EmixilConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = emixillib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = emixillib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = emixillib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_emixild_proto_version_with_sentinel_ping)
