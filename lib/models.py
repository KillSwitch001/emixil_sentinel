...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, emixild):
        golist = emixild.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_emixild(emixild, item)
    ...
    @classmethod
    def import_gobject_from_emixild(self, emixild, rec):
        import decimal
        import emixillib
        import inflection
        ...
        # shim/emixild conversion
        object_hex = emixillib.SHIM_deserialise_from_emixild(object_hex)
        objects = emixillib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from emixild...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from emixild, with every run
        ...
        # get/create, then sync payment amounts, etc. from emixild - Emixild is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(emixild)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from emixild! %s" % e)
            govobj.vote_delete(emixild)
            return (govobj, None)
    ...
    def vote_delete(self, emixild):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(emixild, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, emixild, signal, outcome):
        import emixillib
        ...
        output = emixild.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = emixillib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(emixild, signal)

    def sync_network_vote(self, emixild, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = emixild.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 emixil addr, non-multisig
            if not emixillib.is_valid_emixil_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Emixil address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (emixillib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/EmixilDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import emixillib
        obj_data = emixillib.SHIM_serialise_for_emixild(self.serialise())
    ...
    def prepare(self, emixild):
        try:
            object_hash = emixild.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not emixillib.is_valid_emixil_address(addr, config.network):
    ...
    def hash(self):
        import emixillib
        return emixillib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, emixild):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - emixild.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, emixild):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - emixild.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, emixild):
        now = int(time.time())
        return (self.created_at < (now - emixild.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, emixild):
        if self.is_expired(emixild):
            return False

        return True

    def is_deletable(self, emixild):
        if self.is_expired(emixild):
            return True
