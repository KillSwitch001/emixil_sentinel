...
def has_emixil_conf():
    ...
    valid_emixil_conf = False

    # ensure emixil_conf exists & readable
    #
    # if not, print a message stating that Emixil Core must be installed and
    # configured, including JSONRPC access in emixil.conf
    try:
        f = io.open(config.emixil_conf)
        valid_emixil_conf = True
    except IOError as e:
        print(e)

    return valid_emixil_conf
...
def main():
    ...
    if not has_emixil_conf():
        print("EmixilCore must be installed and configured, including JSONRPC access in emixil.conf")
        sys.exit(1)
