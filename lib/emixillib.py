...
def is_valid_emixil_address(address, network='mainnet'):
    ...
    emixil_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Emixil address
        return False

    if (address_version != emixil_version):
        return False
...
# shims 'til we can fix the emixild side
def SHIM_serialise_for_emixild(sentinel_hex):
    ...
    # superblock => "trigger" in emixild
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # emixild expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    emixild_hex = serialise(obj)
    return emixild_hex


# shims 'til we can fix the emixild side
def SHIM_deserialise_from_emixild(emixild_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(emixild_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('emixil.conf')
